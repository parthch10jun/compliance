...
interface AllControlsModalProps {
  isOpen: boolean;
  onClose: () => void;
  programName: string;
  controls: Control[];
...
export function AllControlsModal({ 
  isOpen, 
  onClose, 
  programName, 
  controls,
  onControlClick 
}: AllControlsModalProps) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-start justify-between p-6 border-b border-[var(--border)]">
          <div className="flex-1">
            <h2 className="text-h2 font-bold text-[var(--foreground)] mb-2">All Controls</h2>
            <p className="text-p2 text-[var(--foreground-muted)]">
              {controls.length} control{controls.length !== 1 ? 's' : ''} in <span className="font-medium text-[var(--foreground)]">{programName}</span>
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {controls.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-p2 text-[var(--foreground-muted)]">No controls found</p>
            </div>
          )}
        </div>
